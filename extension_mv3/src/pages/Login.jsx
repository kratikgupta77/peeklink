...
export default function LoginPage() {
  ...
  async function handleLogin(e) {
    ...
    setStatus("");
    try {
      // Get API base from chrome.storage or use default
      ...
      // Store refresh token
      if (typeof chrome !== "undefined" && chrome.storage) {
        chrome.storage.sync.set({ refreshToken: data.refresh });
      } else {
  ...
  function openDashboard() {
    if (typeof chrome !== "undefined" && chrome.tabs) {
      chrome.tabs.create({ url: "http://127.0.0.1:5173/login" });
    } else {
      window.open("http://127.0.0.1:5173/login", "_blank");
    }
  }
  ...
  return (
    <div style={{ padding: 24, background: "#f9fafb", minHeight: "100vh" }}>
      <div style={{ maxWidth: 400, margin: "0 auto" }}>
        <h2 style={{ marginBottom: 24, fontSize: 24, fontWeight: 700 }}>PeekLink Extension</h2>
